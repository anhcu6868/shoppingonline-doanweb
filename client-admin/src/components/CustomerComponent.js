    ...
    const customers = this.state.customers.map((item) => {
      return (
        <tr key={item._id} className="datatable" onClick={() => this.trCustomerClick(item)}>
          ...
          <td>
            {item.active === 0 ?
              <span className="link btn btn-success" onClick={() => this.lnkEmailClick(item)}>EMAIL</span>
              :
              <span className="link btn btn-danger" onClick={() => this.lnkDeactiveClick(item)}>DEACTIVE</span>}
          </td>
        </tr>
      );
    });
